...
from ...discovery.models import AVVerdict
from .isearch import ISearch


class SearchByAVVerdict(ISearch):
    def __init__(self, s : Optional[str] = None, search_type : Optional[str] = None):
        self.s = s
        self.search_type = search_type

    def get(self) -> List[int]:
        _av_verdict_search, tv = AVVerdict.search(expression=self.s, search_type=self.search_type)
